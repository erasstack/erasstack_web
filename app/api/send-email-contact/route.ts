// src/app/api/send-email-contact/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    const data = await resend.emails.send({
      from: "contact@erasstack.com", // your verified domain
      ...
      html: `
        <div style="font-family: 'Inter', sans-serif; color:hsl(220,30%,15%); line-height:1.6; padding:24px; background-color:hsl(40,20%,98%);">
          <header style="text-align:center; margin-bottom:24px;">
            <h1 style="font-family:'DM Sans', sans-serif; font-weight:700; color:hsl(220,47%,20%); margin:0;">ErasStack Contact Message</h1>
            <p style="color:hsl(215,20%,45%); font-size:14px; margin-top:4px;">New message submitted via website</p>
          </header>
          ...
          <section style="background-color:hsl(0,0%,100%); padding:20px; border-radius:8px; border:1px solid hsl(220,15%,88%); box-shadow:0 6px 16px hsl(220,30%,15% / 0.06);">
            <table style="width:100%; border-collapse:collapse;">
              ...
              <tr>
                <td style="font-weight:600; padding:6px; color:hsl(220,47%,20%);">Subject:</td>
                <td style="padding:6px;">${body.subject || "N/A"}</td>
              </tr>
              <tr>
